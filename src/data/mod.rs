...
impl Dataset {
    ...
    pub fn uprate_to(&mut self, target_year: u32) {
        ...
        for p in &mut self.people {
            ...
            p.state_pension_reported *= factor;
            ...
            p.child_benefit_reported *= factor;
            p.housing_benefit_reported *= factor;
            p.income_support_reported *= factor;
            p.pension_credit_reported *= factor;
            p.child_tax_credit_reported *= factor;
            p.working_tax_credit_reported *= factor;
            p.universal_credit_reported *= factor;
            p.dla_sc_reported *= factor;
            p.dla_m_reported *= factor;
            p.pip_dl_reported *= factor;
            p.pip_m_reported *= factor;
            p.carers_allowance_reported *= factor;
            p.attendance_allowance_reported *= factor;
            p.esa_income_reported *= factor;
            p.esa_contrib_reported *= factor;
            p.jsa_income_reported *= factor;
            p.jsa_contrib_reported *= factor;
        }
