...
impl Simulation {
    ...
    pub fn run(&self) -> SimulationResults {
        ...
        let pr: Vec<PersonResult> = self
            .people
            .par_iter()
            .map(|person| variables::income_tax::calculate(person, &self.parameters))
            .collect();
        person_results = pr;
        ...
        let hr: Vec<HouseholdResult> = self
            .households
            .par_iter()
            .map(|hh| {
                let gross: f64 = hh
